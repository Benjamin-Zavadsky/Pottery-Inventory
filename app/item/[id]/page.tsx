...
export default function ItemPage() {
  ...
  async function handleGenerateDescription() {
    ...
    try {
      ...
      if (!images.length) {
        setAnalyzeError('No photos found to analyze');
        return;
      }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f9f9f9]">
      ...
      <header className="bg-white border-b border-[#e5e5e5] px-4 sm:px-6 py-3 sm:py-4 flex items-center gap-3 sticky top-0 z-10">
        ...
        <span className="text-xs sm:text-sm font-mono text-[#6b6b6b] flex-1 truncate">
          {item.sku}
        </span>
        {user && (
              ...
              <>
                {item.status === 'Archived' ? (
                  ...
                  <button
                    ...
                    className="text-sm text-[#6b6b6b] hover:text-red-500 transition-colors hidden sm:block"
                  >
                    Archive
                  </button>
                )}
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full">
            ...
            <div className="flex flex-col gap-2">
              {Object.entries(suggestions)
                ...
                .map(([key, value]) => (
                  <div
                    key={key}
                    className="flex items-start gap-3 bg-white border border-[#e5e5e5] rounded-xl p-3"
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] text-[#aaa] uppercase tracking-wider mb-0.5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-8 lg:px-6 lg:py-8">
            ...
            {user && !editing && (
              <div className="px-4 sm:px-0 pb-4 mt-2 sm:hidden">
                {item.status === 'Archived' ? (
                  <button
                    onClick={handlePermanentDelete}
                    className="w-full border border-red-200 bg-white rounded-xl py-2.5 text-sm text-red-500 hover:border-red-400 hover:text-red-700 transition-colors"
                  >
                    Permanently delete
                  </button>
                ) : (
                  <button
                    onClick={handleArchive}
                    className="w-full border border-[#e5e5e5] bg-white rounded-xl py-2.5 text-sm text-[#6b6b6b] hover:border-red-300 hover:text-red-500 transition-colors"
                  >
                    Archive this piece
                  </button>
                )}
          ...
          <div className="px-4 sm:px-6 lg:px-0 py-4 lg:py-0 flex flex-col gap-4">
            {editing ? <EditForm form={form} set={set} /> : <ViewInfo item={item} />}
          </div>
        </div>
      </main>
    </div>
    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-3">
        ...
        {item.case_id && (
          <InfoField
            label="Display Case"
            value={CASES.find((c) => c.id === item.case_id)?.name ?? item.case_id}
          />
        )}
        {item.date_acquired && (
          <InfoField
            label="Date Acquired"
            value={new Date(item.date_acquired).toLocaleDateString()}
          />
        )}
        {item.location_acquired && (
          <InfoField label="Acquired From" value={item.location_acquired} />
        )}
        ...
        {item.appraised_value != null && (
          <InfoField label="Appraised Value" value={`$${item.appraised_value.toLocaleString()}`} />
        )}
        {item.acquisition_cost != null && (
          <InfoField
            label="Acquisition Cost"
            value={`$${item.acquisition_cost.toLocaleString()}`}
          />
        )}
        {item.museums_comparable && (
          <InfoField label="Comparable Museums" value={item.museums_comparable} />
        )}
      </div>
...
function EditForm({
  ...
  form: Partial<PotteryItem>;
  set: (f: string, v: unknown) => void;
}) {
  const inp =
    'w-full border border-[#e5e5e5] rounded-xl px-3 py-2.5 text-sm bg-white focus:outline-none focus:border-[#111] transition-colors';
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <EF label="Use / Function">
          <input
            value={form.use_function ?? ''}
            onChange={(e) => set('use_function', e.target.value)}
            className={inp}
          />
        </EF>
        <EF label="Tribe / Culture">
          <input
            value={form.tribe_culture ?? ''}
            onChange={(e) => set('tribe_culture', e.target.value)}
            className={inp}
          />
        </EF>
        <EF label="Dimensions">
      ...
      <div className="grid grid-cols-2 gap-3">
        <EF label="Location Acquired">
          <input
            value={form.location_acquired ?? ''}
            onChange={(e) => set('location_acquired', e.target.value)}
            className={inp}
          />
        </EF>
        <EF label="Seller / Donator">
          <input
            value={form.seller_donator ?? ''}
            onChange={(e) => set('seller_donator', e.target.value)}
            className={inp}
          />
        </EF>
        <EF label="Appraised Value ($)">
          <input
            type="number"
            value={form.appraised_value ?? ''}
            onChange={(e) =>
              set('appraised_value', e.target.value ? parseFloat(e.target.value) : null)
            }
            className={inp}
          />
        </EF>
        <EF label="Acquisition Cost ($)">
          <input
            type="number"
            value={form.acquisition_cost ?? ''}
            onChange={(e) =>
              set('acquisition_cost', e.target.value ? parseFloat(e.target.value) : null)
            }
            className={inp}
          />
        </EF>
      ...
      <EF label="Comparable Museums">
        <input
          value={form.museums_comparable ?? ''}
          onChange={(e) => set('museums_comparable', e.target.value)}
          className={inp}
        />
      </EF>
      <EF label="Research Notes">
        <textarea
